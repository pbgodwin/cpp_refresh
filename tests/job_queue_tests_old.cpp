...
TEST_CASE("JobQueue: Pop-while-steal produces no lost or dup tasks")
{
    ...
    std::mutex bag_mutex; // Added mutex
    std::atomic<bool> stop{false};
    Task tmp;

    std::thread thief([&]{
        Task tmp_thief; // Use a local variable for the thief
        while (!stop.load(std::memory_order_acquire))
        {
            if (q.steal(q, tmp_thief)) 
            {
                std::lock_guard<std::mutex> lock(bag_mutex);
                // Check size before inserting to ensure we don't overshoot CAP
                // due to race with main thread's loop condition.
                if (bag.size() < CAP)
                {
                    bag.insert(tmp_thief);
                }
            }
            else
            {
                std::this_thread::yield(); // Yield if steal failed
            }
        }
    });

    Task tmp_main; // Use a local variable for the main thread
    while (true)
    {
        bool should_break = false;
        {
            std::lock_guard<std::mutex> lock(bag_mutex);
            if (bag.size() >= CAP) // Check size under lock
            {
                should_break = true;
            }
        }
        ...
        {
            std::lock_guard<std::mutex> lock(bag_mutex);
            // Re-check size under lock before inserting, as thief might have added items.
            if (bag.size() < CAP)
            {
                bag.insert(tmp_main);
            }
        ...
        {
            std::this_thread::yield(); // Yield if pop failed
        }
    }
