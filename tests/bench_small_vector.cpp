...
TEST_CASE("push_back vs std::vector", "[bench]") {
    ...
    BENCHMARK("SmallVector: SmallVector<N=8> push_back N ints") {
    ...
    BENCHMARK("SmallVector: SmallVector<N=1'000> push_back N ints") {
        SmallVector<int, 1'000> sv;
        for (std::size_t i = 0; i < N; ++i) sv.push_back(static_cast<int>(i));
        return sv.size();               // return something so Catch2 can't dead-strip
    };

    BENCHMARK("SmallVector: std::vector push_back N ints") {
        std::vector<int> v;
